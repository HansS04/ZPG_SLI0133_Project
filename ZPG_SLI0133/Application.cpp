...
void Application::setupScene3(Scene* scene) {
    ...
    scene->getLight().setPosition(glm::vec3(0.0f, 20.0f, 0.0f));
    scene->getLight().setColor(glm::vec3(0.1f, 0.1f, 0.2f));
    ...
    for (int i = 0; i < numFireflies; ++i) {
        float x = (std::rand() / (float)RAND_MAX) * (FOREST_WIDTH - PATH_WIDTH) + PATH_WIDTH;
        if (std::rand() % 2 == 0) x = -x;

        float y = (std::rand() / (float)RAND_MAX) * 2.0f + 0.5f;

        float z = (std::rand() / (float)RAND_MAX) * SCENE_LENGTH - (SCENE_LENGTH / 2.0f);

        scene->addAdditionalLight(glm::vec3(x, y, z), fireflyColor, con, lin, quad);
    }
