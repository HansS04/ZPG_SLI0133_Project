...
void main() {
    ...
    if (dot(norm, lightDir) > 0.0) {
        ...
        float diff_intensity = dot(norm, lightDir);
        diffuse = vec4(lightColor * diff_intensity * u_Color.rgb, 1.0);
        diffuse *= attenuation;

        vec3 reflectDir = reflect(-lightDir, norm);
        float spec_intensity = pow(max(dot(viewDir, reflectDir), 0.0), SPECULAR_SHININESS);
        
        specular = vec4(lightColor * spec_intensity * vec3(1.0, 1.0, 1.0), 1.0);
