...
void Application::processInput(GLFWwindow* window, float deltaTime) {
    ...
    CameraController& controller = scene->getCameraController();

    if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) {
        controller.processKeyboard(CameraController::Movement::FORWARD, deltaTime);
    }
    if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) {
        controller.processKeyboard(CameraController::Movement::BACKWARD, deltaTime);
    }
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) {
        controller.processKeyboard(CameraController::Movement::LEFT, deltaTime);
    }
    if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) {
        controller.processKeyboard(CameraController::Movement::RIGHT, deltaTime);
    }

    if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS) loadScene(1);
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS) loadScene(2);
    if (glfwGetKey(window, GLFW_KEY_3) == GLFW_PRESS) loadScene(3);
    if (glfwGetKey(window, GLFW_KEY_0) == GLFW_PRESS) loadScene(0);
}
...
void Application::cursor_position_callback(GLFWwindow* window, double xpos, double ypos) {
    ...
    float yoffset = (float)ypos - app->lastY;

    app->lastX = (float)xpos;
    app->lastY = (float)ypos;

    if (app->rightButtonPressed && app->scene) {
        app->scene->getCameraController().processMouseMovement(xoffset, yoffset, true);
    }
}
...
void Application::scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
    ...
    if (app && app->scene) {
        app->scene->getCameraController().processScroll((float)yoffset);
    }
}
...
float SCALE_EARTH = 0.1f;
float SCALE_MOON = SCALE_EARTH / 20.0f;
